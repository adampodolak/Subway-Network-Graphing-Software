...
class Dijkstras(PathfinderAlgorithm):
    def findpath(self, graph, s1, s2):
        ...
        q = [graph.edges[s1-1]]
        distTo = []
        # qfollow and q indexes are connected.
        # qfollow allows access to distTo[old node]
        ...
        while q:
            ...
            index = int(q[0].id)-1
            # distTo[new node] =
            # time to new node from old node + distTo[old node]
            findTime = q[0].neighbours.index(int(qfollow[0].id))
            if distTo[index] > int(
                    q[0].time[findTime]) + distTo[int(qfollow[0].id)-1]:
                distTo[index] = int(
                        q[0].time[findTime]) + distTo[int(qfollow[0].id)-1]
    ...
    def find_neighbours(self, graph, q, qfollow):
